...
import { doc, getDoc, collection, getDocs, Timestamp, query, orderBy, updateDoc, deleteDoc } from 'firebase/firestore';
import Link from 'next/link';

import { PageHeader } from '@/components/shared/PageHeader';
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Pencil, Loader, Trash2 } from 'lucide-react';
import { PropertyExpenses } from '@/components/properties/PropertyExpenses';
import { PropertyIncome } from './PropertyIncome';
import { db } from '@/lib/firebase';
import { type Property, type ActualExpense, type Income, type ExpectedExpense, type Wallet, type ExpenseCategory, type IncomeCategory, type Liability, type ExpenseSubcategory, type IncomeSubcategory } from '@/lib/types';
...
export function PropertyDetail({ id }: { id: string }) {
  ...
  const fetchPageData = React.useCallback(async () => {
    ...
    try {
        ...
        const expectedExpensesList = expectedExpensesSnapshot.docs.map(docSnap => {
            const expense = { id: docSnap.id, ...docSnap.data() } as ExpectedExpense;
            // Backwards compatibility for old month/year format
            ...
            return {
                ...expense,
                date: (expense.date as any).toDate ? (expense.date as any).toDate().toISOString() : expense.date,
            };
        });
  ...
  const handleDeleteProperty = async () => {
    ...
    try {
      ...
      if (!incomesSnap.empty || !expensesSnap.empty) {
        toast({
          title: "No se puede eliminar",
          description: "Esta cuenta tiene transacciones asociadas. Mueva o elimine las transacciones antes de eliminar la cuenta.",
          variant: "destructive",
          duration: 6000,
        });
        setIsDeleting(false);
      ...
      toast({
        ...
        description: `La cuenta "${property.name}" ha sido eliminada exitosamente.`,
        variant: "destructive"
      });
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <PageHeader title={property.name}>
        <Button asChild variant="outline">
         ...
         <Button variant="destructive-outline" onClick={() => setIsDeleteDialogOpen(true)} disabled={isDeleting}>
     ...
     <ConfirmDeleteDialog
        ...
        title={`¿Eliminar la cuenta "${property?.name}"?`}
        description="Esta acción es permanente. Solo puedes eliminar una cuenta si no tiene ingresos o gastos asociados. ¿Estás seguro?"
      />
    </>
  );
}
