...
import { type Liability, type LiabilityPayment, type Wallet, type ExpenseCategory, type Property, type Income, type ExpenseSubcategory } from '@/lib/types';
            ...
            {payments.length > 0 ? payments.map(payment => (
                <Card key={payment.id} className="p-4">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="font-semibold">{format(new Date(payment.date), 'PP', { locale: es })}</p>
                            <p className="text-sm text-muted-foreground">Cuenta: {properties.find(p => p.id === payment.propertyId)?.name || 'N/A'}</p>
                            <p className="text-sm text-muted-foreground">Billetera: {wallets.find(w => w.id === payment.walletId)?.name || 'N/A'}</p>
                        </div>
                    ...
                    {payment.notes && <p className="text-sm text-muted-foreground mt-2 pt-2 border-t">{payment.notes}</p>}
