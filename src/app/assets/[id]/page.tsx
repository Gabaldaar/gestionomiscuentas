...
import { type Asset, type AssetCollection, type Wallet, type Property, type IncomeCategory, type Expense, type IncomeSubcategory } from '@/lib/types';
            ...
            {collections.length > 0 ? collections.map(collection => (
                <Card key={collection.id} className="p-4">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="font-semibold">{format(new Date(collection.date), 'PP', { locale: es })}</p>
                            <p className="text-sm text-muted-foreground">Cuenta: {properties.find(p => p.id === collection.propertyId)?.name || 'N/A'}</p>
                            <p className="text-sm text-muted-foreground">Billetera: {wallets.find(w => w.id === collection.walletId)?.name || 'N/A'}</p>
                        </div>
                    ...
                    {collection.notes && <p className="text-sm text-muted-foreground mt-2 pt-2 border-t">{collection.notes}</p>}
